...
\small
This document has been published as a vignette of the \proglang{R} package \pkg{tscount}. Last update: September~2017. An earlier version has been published as a discussion paper in February 2015 (\doi{10.17877/DE290R-7239}). A recent version has been conditionally accepted for publication in \emph{Journal of Statistical Software}. Please cite this manuscript as:\\[0.5em]
Tobias Liboschik, Konstantinos Fokianos \& Roland Fried. ``tscount: An R Package for Analysis of Count Time Series Following Generalized Linear Models''. To appear in \emph{Journal of Statistical Software}.
}}
...
@
The possibility of a time dependent dispersion coefficient does not improve the fit for this data example (according to the AIC, which is \Sexpr{round(campyaic_variablesigma,2)} compared to \Sexpr{round(campyaic_constantsigma,2)} for a model with constant overdispersion coefficient) but might be quite useful for other data examples. However, it is clear that such a complex model yields more uncertainty of the parameter estimations (i.e., larger standard errors, which are not shown here).
